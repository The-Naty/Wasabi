...
export interface IAuthService {
  registerUserAndSignToken(
    userData: Partial<User>
  ): Promise<{ token: string; newUser: User }>;
  loginUserAndSignToken(userData: {
    email?: string;
    username?: string;
    password: string;
  }): Promise<{ user: UserWithInfo; token: string }>;
}
...
export class AuthService implements IAuthService {
  ...
  public async registerUserAndSignToken(
    ...
    const { phoneNumber, verificationCode } = userData;
  ...
  public async loginUserAndSignToken(userData: {
    email?: string;
    username?: string;
    password: string;
  }): Promise<{ user: UserWithInfo; token: string }> {
